...
const Groups = () => {
    ...
    return (
        <div className="flex flex-col gap-6 p-4">
            ...
            <section>
                <h3 className="text-xs font-bold text-[var(--text-sub)] uppercase tracking-widest mb-3 px-1">Discover</h3>
                <div className="grid grid-cols-1 gap-3">
                    {otherGroups?.map(group => (
                        <GroupItem key={group._id} group={group} onJoin={() => handleJoin(group._id)} />
                    ))}
                    {otherGroups?.length === 0 && <p className="text-center text-sm text-[var(--text-sub)] py-8">No groups found to join.</p>}
                </div>
            </section>
            ...
            >
                <div className="p-6 flex flex-col gap-5">
                    <div className="flex justify-between items-center">
                        <h3 className="m-0 text-xl font-black text-[var(--text-main)]">Create Community</h3>
                        <button onClick={() => setShowCreate(false)} className="bg-[var(--surface-2)] border-0 rounded-full w-8 h-8 flex items-center justify-center cursor-pointer text-[var(--text-main)]">✕</button>
                    </div>
                    
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-bold text-[var(--text-sub)] px-1 uppercase tracking-wider">Group Name</label>
                            <input 
                                type="text" 
                                value={newGroup.name}
                    ...
                    <button 
                        ...
                        className="w-full bg-[#808bf5] text-white py-4 rounded-3xl font-black shadow-xl shadow-indigo-200/40 border-0 cursor-pointer hover:opacity-95 transition"
                    >
                        Create Group
                    </button>
                </div>
