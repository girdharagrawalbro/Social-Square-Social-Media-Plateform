...
const Explore = () => {
  ...
  useEffect(() => {
    // Shuffle the video pool on mount
  ...
  return (
    <div className="w-full">
      <div className="sticky top-0 z-20 px-2 sm:px-4 py-4 bg-[var(--surface-1)] border-[var(--border-color)] mb-6">
      ...
      <div className="px-2 sm:px-4 pb-8">
        <div className="grid grid-cols-2 md:grid-cols-3 max-w-4xl mx-auto gap-1.5 sm:gap-3">
          {videos.map((vid, idx) => (
            <div
              key={vid.id}
              className={`relative rounded-2xl overflow-hidden bg-[var(--surface-2)] cursor-pointer group transition-transform active:scale-95`}
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-4 flex flex-col justify-end">
                <span className="text-[10px] uppercase font-bold text-white/80 tracking-widest">{vid.creator}</span>
                <p className="m-0 text-white font-bold text-sm truncate">{vid.title}</p>
              </div>
            </div>
          ))}
        </div>
