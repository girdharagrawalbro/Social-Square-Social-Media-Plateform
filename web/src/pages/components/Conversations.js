...
const MessagePreview = ({ messageText, conversationId, recipientId }) => {
    ...
    useEffect(() => {
        if (!messageText) { setDecryptedText(''); return; }
        const unescaped = unescapeHtml(messageText);
        if (!unescaped.startsWith('{"ciphertext":')) {
                                            ...
                                            <div className="flex flex-col justify-center flex-1 min-w-0">
                                                ...
                                                <div className="flex justify-between items-center gap-2">
                                                    <p className={`p-0 m-0 text-sm truncate flex-1 leading-tight ${isUnread ? 'font-medium text-gray-800 dark:text-gray-100' : 'text-gray-500 dark:text-gray-400 font-normal'}`}>
                                                        ...
                                                        <MessagePreview messageText={conv.lastMessage?.message} conversationId={conv._id} recipientId={displayId} />
                                                    </p>
                                                    {convUnread > 0 && (
