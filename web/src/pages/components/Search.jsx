...
const Search = ({ onClose, desc = true }) => {
    ...
    // token is stored in-memory in Zustand; prefer that over localStorage
    ...
    const doSearch = async (term) => {
        ...
        try {
            ...
            if (token) {
                ...
                // Token not present in-memory; warn and allow unauthenticated requests where supported
                console.warn('Search: no auth token found in store; sending unauthenticated request');
            }
            ...
            if (!res.ok) {
                // Try to capture server-provided message
                let errText = '';
                try { errText = await res.text(); } catch (e) { /* ignore */ }
                const msg = `Search request failed: ${res.status} ${res.statusText}${errText ? ` - ${errText}` : ''}`;
                console.error(msg);
                setSearchError(msg);
                ...
                <div className={`absolute left-0 w-[90vw] sm:w-[500px] md:w-[600px] lg:w-full bg-[var(--surface-1)] rounded-2xl z-50 overflow-y-auto mt-2 shadow-[0_12px_45px_rgba(0,0,0,0.15)] dark:shadow-[0_12px_45px_rgba(0,0,0,0.6)] backdrop-blur-xl ${!isFocused && !searchTerm ? 'hidden' : 'block'} transition-all duration-300`} style={{ top: '100%', maxHeight: 'min(780px, calc(100vh - 80px))', overscrollBehavior: 'contain', border: '1px solid var(--border-color)' }}>
                    ...
                    {searchTerm && (
                        <div className="pt-3 px-2 pb-6">
                            {searchError && (
                                <div className="px-3 py-2 mb-3 rounded bg-rose-50 text-rose-700 text-sm">{searchError}</div>
                            )}
