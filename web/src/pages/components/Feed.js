...
const Feed = ({ activeMood = null }) => {
    ...
    // ── Offline Cache State & Effects ────────────────────────────────────────
    const [offlinePosts, setOfflinePosts] = useState([]);

    useEffect(() => {
        const loadOfflineFeed = async () => {
            try {
                const cached = await dbService.getCache('feed_posts');
                if (cached && Array.isArray(cached) && cached.length > 0) {
                    setOfflinePosts(cached);
                }
            } catch (err) {
                console.error("Failed to load offline feed cache:", err);
            }
        };
        loadOfflineFeed();
    }, []);
    ...
    useEffect(() => {
        const fetched = feedQuery.data?.pages?.[0]?.posts;
        if (fetched && fetched.length > 0) {
            dbService.setCache('feed_posts', fetched.slice(0, 30));
        }
    }, [feedQuery.data]);
